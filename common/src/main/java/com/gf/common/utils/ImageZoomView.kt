...
class ImageZoomView : AppCompatImageView, View.OnTouchListener,
    GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {

    companion object {
        // Estados de la imagen
        private const val TAG = "ImageZoomView"
        const val NONE = 0
        const val DRAG = 1
        const val ZOOM = 2
    }
    var isEndScrolling = false
    var canScroll : ((Boolean) -> Unit)? = null
    ...
    var mMinScale = 1f
    var mMaxScale = 5f
    private val isZoomed
        get () = mSaveScale > mMinScale
    ...
    fun fitToScreen() {
        ...
        var ySpace = (viewHeight.toFloat()
                - scale * imageHeight.toFloat())
        var xSpace = (viewWidth.toFloat()
                - scale * imageWidth.toFloat())
        ySpace /= 2.toFloat()
        xSpace /= 2.toFloat()
        mMatrix!!.postTranslate(xSpace, ySpace)
        origWidth = viewWidth - 2 * xSpace
        origHeight = viewHeight - 2 * ySpace
        imageMatrix = mMatrix
    }
    ...
    fun fixTranslation() {
        ...
        isEndScrolling = fixTransX != 0f
        if (mode == ZOOM)
            canScroll?.invoke(false)

        else if (!isZoomed)
            canScroll?.invoke(true)

        else
            canScroll?.invoke(isEndScrolling)
